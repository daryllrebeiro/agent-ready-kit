# AgentReady — Target Architecture (ASPIRATION, NOT CURRENT STATE)

> **This document describes where the project is headed, not what exists.**
> For what is wired and tested today, see `FEATURES.md` (repo root).
> Preserved verbatim from the former `FEATURES.md` on 2026-09-11 so no
> design thinking is lost; claims below (GA, 93%, hosted services) are
> **not** statements about the current codebase.
