...
class OnboardingWizard:
    ...
    def __init__(
        ...
        simulator: AgentPersonaSimulator | None = None,
        repository: PostgresRLSRepository | None = None,
    ):
    ...
    def execute_onboarding_flow(
        ...
        # Step 3: Competitor Benchmark Comparison. Previously this step
        # fabricated results without running anything; now it executes the
        # real benchmark engine when competitors are supplied, and records
        # an honest self-baseline (no fabricated comparison) otherwise.
        from packages.core.competitors.benchmark import CompetitorBenchmarkEngine

        competitors = competitor_domains or []
        if competitors:
            benchmark = CompetitorBenchmarkEngine().compare_domains(
                target_domain, competitors, dry_run=True
            )
            step3 = {
                "step": 3,
                "name": "Competitive Benchmark & Badge Issuance",
                "competitors_compared": len(competitors),
                "win_status": benchmark.get("win_status"),
            ...
            step3 = {
                "step": 3,
                "name": "Self-Baseline & Badge Issuance",
                "competitors_compared": 0,
                "badge_url": f"/api/badge?domain={target_domain}",
                "status": "COMPLETED_NO_COMPARISON",
            }
