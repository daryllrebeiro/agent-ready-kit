...
class LLMsGenerator:
    ...
    def _assert_safe_target(url: str) -> None:
        """P0.1: refuse private/metadata targets before any fetch."""
    ...
    def parse_sitemap(self, sitemap_url: str, max_urls: int = 50) -> list[str]:
        ...
        urls: list[str] = []
        try:
            self._assert_safe_target(sitemap_url)
        except Exception:
            return urls
        try:
    ...
    def extract_page_summary(self, url: str) -> dict[str, str]:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                raw_content = desc_tag.get("content", "") if desc_tag else ""
    ...
    def generate_llms_txt(
        ...
        if not pages:
            lines.append(
                "- [Overview](https://example.com/docs): Comprehensive product documentation and reference guides."
            )
        else:
            for page in pages:
