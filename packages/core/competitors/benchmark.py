...
class CompetitorBenchmarkEngine:
    ...
    def compare_domains(
            ...
            for cited in res.cited_domains:
                for tracked_domain in all_domains:
                    if tracked_domain in cited or cited in tracked_domain:
                        ...
                        provider_citations[tracked_domain][prov] = (
                            provider_citations[tracked_domain].get(prov, 0) + 1
                        )
        ...
        win_status = (
            "WINNING"
            if target_citations > max_competitor_citations
            else "TIED"
            if target_citations == max_competitor_citations
            else "LOSING"
        )
            ...
            [
                {
                    ...
                    "score": scores[u].overall_score,
                    "grade": scores[u].grade,
                }
                for u in all_urls
            ],
            key=_rank_score,
