...
def handle_scan_command(args: argparse.Namespace) -> int:
    ...
    from packages.core.observability.logger import get_structured_logger

    console = Console()
    err_console = Console(stderr=True)
    logger = get_structured_logger("agentready.cli")
    scorer = Scorer(timeout_seconds=args.timeout)
    logger.info(f"cli scan target={args.url}")

    if not args.json:
        console.print(f"[dim]Scanning [bold white]{args.url}[/bold white] for agent-readiness...[/dim]")

    # P0.1: refuse unsafe targets before any network call. score_url()
    # degrades to a garbage low score on fetch failure; the CLI must not
    # present that as a successful scan of a private/metadata endpoint.
    try:
        scorer.resolve_and_validate(args.url)
    except Exception as e:
        err_console.print(f"[bold red]Refusing to scan unsafe target:[/bold red] {e}")
        return 2
...
def handle_generate_command(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "languages", None):
        ...
        bundle = i18n_gen.generate_multilingual_bundle(
            site_name, target_url or "https://example.com", languages=langs
        )
        written = i18n_gen.write_bundle_to_disk(bundle, args.output_dir or "./public")
        console.print(
            f"[bold green][OK] Successfully generated multilingual /llms.txt suite in '{args.output_dir or './public'}':[/bold green]"
        )
    ...
    console.print("[bold green][OK] Successfully generated agent-ready files:[/bold green]")
...
def handle_probe_command(args: argparse.Namespace) -> int:
    ...
    # Phase 17 1.3: full guarded pipeline on the CLI path (budget -> cache
    # -> breaker -> provider -> DLQ). Tenant resolves from
    # AGENTREADY_TENANT_ID else a local default; the shared cache backend
    # (real Redis when reachable) makes state visible across invocations.
    ...
    pipeline = ProbePipeline(tenant_id=tenant_id, target_url=args.url)
    ...
    with TraceContext(tenant_id=tenant_id):
        try:
            suite_results = pipeline.run_suite(
                prober,
                STANDARD_PROBE_PROMPTS[: args.max_prompts],
            ...
            logger.warning(f"cli probe budget exhausted for tenant {tenant_id}")
            return 3
        logger.info(f"cli probe suite completed for {target_domain}")

    total_probes = 0
    total_citations = 0

    table = Table(
        title=f"Multi-Model LLM Probing Results: {target_domain}", show_header=True, header_style="bold cyan"
    )
    ...
    samples = [
        {
            "score": {
                "overall_score": 92.0,
                "components": [
                    {"name": "llms_txt", "score": 95},
                    {"name": "structured_data", "score": 90},
                    {"name": "token_bloat", "score": 85},
                    {"name": "bot_permissions", "score": 95},
                ],
            },
            "citation_rate": 0.85,
        },
        {
            "score": {
                "overall_score": 84.0,
                "components": [
                    {"name": "llms_txt", "score": 80},
                    {"name": "structured_data", "score": 85},
                    {"name": "token_bloat", "score": 80},
        ...
        },
        {
            "score": {
                "overall_score": 68.0,
                "components": [
                    {"name": "llms_txt", "score": 50},
                    {"name": "structured_data", "score": 70},
                    {"name": "token_bloat", "score": 75},
        ...
        },
        {
            "score": {
                "overall_score": 45.0,
                "components": [
                    {"name": "llms_txt", "score": 0},
                    {"name": "structured_data", "score": 50},
                    {"name": "token_bloat", "score": 60},
                    {"name": "bot_permissions", "score": 60},
                ],
            },
            "citation_rate": 0.20,
        },
        {
            "score": {
                ...
                "components": [
                    {"name": "llms_txt", "score": 0},
                    {"name": "structured_data", "score": 10},
                    {"name": "token_bloat", "score": 40},
                    {"name": "bot_permissions", "score": 20},
                ],
            },
            "citation_rate": 0.05,
        },
...
def handle_fix_command(args: argparse.Namespace) -> int:
    ...
    console.print(
        f"[dim]Analyzing [bold white]{args.url}[/bold white] and generating remediation files...[/dim]"
    )
    ...
    console.print(
        f"[bold green][OK] Successfully generated drop-in remediation bundle in '{out_dir}':[/bold green]"
    )
...
def handle_compare_command(args: argparse.Namespace) -> int:
    ...
    console.print(
        f"[dim]Running head-to-head citation benchmark for [bold white]{args.url}[/bold white] vs {args.competitors}...[/dim]"
    )
    ...
    console.print(
        f"[bold white]Benchmark Status:[/bold white] [bold {'green' if res['win_status'] == 'WINNING' else 'yellow' if res['win_status'] == 'TIED' else 'red'}]{res['win_status']}[/]"
    )
    ...
    for idx, item in enumerate(res["readiness_ranking"], 1):
        ...
        console.print(
            f"  {idx}. [bold]{item['domain']}[/bold] — Score: {item['score']:.1f} ({item['grade']}) | Citations: {c_count}"
        )
    return 0
...
def handle_batch_command(args: argparse.Namespace) -> int:
    ...
    if args.output:
        ...
        console.print(
            f"[bold green][OK] Exported {len(scores)} results to [bold cyan]{args.output}[/bold cyan][/bold green]"
        )
    else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"AgentReady CLI {CLI_VERSION} (Algorithm: {ALGORITHM_VERSION})",
    )
    ...
    scan_parser = subparsers.add_parser(
        "scan", help="Scan a website for AI agent readiness and citation optimization"
    )
    scan_parser.add_argument("url", help="URL of the website or endpoint to scan")
    scan_parser.add_argument(
        "--min-score",
        type=float,
        help="Minimum score threshold (0-100) for CI/CD gates; exits with code 1 if failed",
    )
    scan_parser.add_argument(
        "--json", action="store_true", help="Output full report as machine-readable JSON"
    )
    scan_parser.add_argument(
        "--pr-comment", action="store_true", help="Output report formatted as a GitHub PR markdown comment"
    )
    scan_parser.add_argument("--output", "-o", help="Path to write report output")
    scan_parser.add_argument(
        "--timeout", type=float, default=10.0, help="HTTP request timeout in seconds (default: 10.0)"
    )

    # Probe command (v1 core, budget-enforced)
    probe_parser = subparsers.add_parser(
        "probe", help="Probe LLM models to test live citation behavior (budget-enforced)"
    )
    probe_parser.add_argument("url", help="Target URL or domain to test for citations")
    probe_parser.add_argument(
        "--max-prompts", type=int, default=3, help="Number of benchmark discovery prompts to run"
    )
    probe_parser.add_argument(
        "--dry-run", action="store_true", help="Simulate probes without consuming LLM API keys"
    )

    # Generate command (v1 core)
    gen_parser = subparsers.add_parser(
        "generate", help="Generate compliant llms.txt and structured metadata templates"
    )
    ...
    gen_parser.add_argument(
        "--languages",
        help="Comma-separated language codes for multilingual llms.txt suite (e.g. en,es,ja,de,fr,zh)",
    )
    gen_parser.add_argument(
        "--max-pages", type=int, default=20, help="Maximum number of sitemap pages to include (default: 20)"
    )
    gen_parser.add_argument(
        "--output-dir", default=".", help="Directory to save generated files (default: current directory)"
    )
    ...
    auth_parser = subparsers.add_parser(
        "auth", help="Manage AgentReady API keys and authentication credentials"
    )
...
def handle_report_command(args: argparse.Namespace) -> int:
    ...
    console.print(
        f"[dim]Compiling Executive AI Agent Health Report for [bold white]{args.url}[/bold white]...[/dim]"
    )
    ...
    if args.output:
        with open(args.output, "w", encoding="utf-8") as f:
            f.write(report_md)
        console.print(
            f"[bold green][OK] Successfully saved Executive Health Report to [bold cyan]{args.output}[/bold cyan][/bold green]"
        )
    else:
        console.print(f"\n{report_md}")
    return 0
...
def handle_simulate_command(args: argparse.Namespace) -> int:
    ...
    console.print(
        f"[bold white]Overall Persona Compatibility:[/bold white] [bold green]{res['overall_compatibility']}/100[/bold green]\n"
    )

    console.print("[bold]Agent Archetype Compatibility Breakdown:[/bold]")
    for p in res["personas"].values():
        badge_color = (
            "green" if p["status"] == "EXCELLENT" else "yellow" if p["status"] == "MODERATE" else "red"
        )
        console.print(
...
def handle_auth_command(args: argparse.Namespace) -> int:
    ...
    if args.auth_action == "login":
        ...
        console.print(
            f"[bold green][OK] Successfully authenticated and saved API key: [bold cyan]{masked}[/bold cyan][/bold green]"
        )
        return 0
        ...
        if key:
            ...
            console.print(
                f"[bold white]Authenticated as API Key:[/bold white] [bold cyan]{masked}[/bold cyan]"
            )
        else:
            console.print(
                "[dim]No API key currently stored. Run `agentready auth login --key <key>` to log in.[/dim]"
            )
        return 0
