...
def test_multi_provider_simultaneous_outage_resilience():
    ...
    results.append(
        ProbeResult(
            provider="anthropic",
            prompt=prompt,
            raw_response="Cited: example.com",
            cited_domains=[target_domain],
            latency_ms=60.0,
        )
    )
...
def test_database_level_competitor_benchmark_tenant_isolation():
    ...
    cur.execute(
        "INSERT INTO competitor_benchmarks VALUES ('b_1', 'tenant_acme', 'acme.com', 'rival.com', 75.0, 'private')"
    )
    # Insert Tenant Beta benchmark (Private)
    cur.execute(
        "INSERT INTO competitor_benchmarks VALUES ('b_2', 'tenant_beta', 'beta.com', 'rival.com', 40.0, 'private')"
    )
    conn.commit()
